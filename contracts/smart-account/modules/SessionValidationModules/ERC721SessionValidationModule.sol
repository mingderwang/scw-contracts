...
/**
 * @title ERC721 Session Validation Module for Biconomy Smart Accounts.
 * @dev Validates userOps for ERC721 transfers and approvals using a session key signature.
 *         - Recommended to use with standard ERC721 tokens only
...
contract ERC721SessionValidationModule is ISessionValidationModule {
    /**
    ...
    function validateSessionParams(
        ...
        require(destinationContract == token, "ERC721SV Invalid Token");
        require(callValue == 0, "ERC721SV Non Zero Value");
        ...
        require(recipient == recipientCalled, "ERC721SV Wrong Recipient");
        require(amount <= maxAmount, "ERC721SV Max Amount Exceeded");
        return sessionKey;
    }
    ...
    function validateSessionUserOp(
...

        {
            ...
            //we expect data to be the `IERC721.transfer(address, uint256)` calldata
        ...
        if (recipientCalled != recipient) {
            revert("ERC721SV Wrong Recipient");
        }
        if (amount > maxAmount) {
            revert("ERC721SV Max Amount Exceeded");
        }
