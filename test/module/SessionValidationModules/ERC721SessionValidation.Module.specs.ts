...
describe("SessionKey: ERC721 Session Validation Module", async () => {
  const [
    deployer,
    smartAccountOwner,
    alice,
  ...
  const setupTests = deployments.createFixture(async ({ deployments }) => {
    ...
    const erc721SessionModule = await (
      await ethers.getContractFactory("ERC721SessionValidationModule")
    ).deploy();

    const { sessionKeyData, leafData } = await getERC721SessionKeyParams(
      sessionKey.address,
      mockToken.address,
      charlie.address,
      maxAmount,
    ...
    const vulnerableerc721SessionModule = await (
      await ethers.getContractFactory("VulnerableERC721SessionValidationModule")
    ).deploy();
    ...
    return {
      ...
      erc721SessionModule: erc721SessionModule,
      sessionKeyData: sessionKeyData,
      leafData: leafData,
      merkleTree: merkleTree,
      vulnerableerc721SessionModule: vulnerableerc721SessionModule,
    };
  });

  const makeERC721TransferUserOp = async function (
    token: string,
    amount: BigNumber,
    recipient: string,
    txnValue: BigNumber,
    ...
    const transferUserOp = await makeEcdsaSessionKeySignedUserOp(
      ...
      0,
      0,
      testParams.erc721SessionModule.address,
  ...
  it("MOVED: should be able to process Session Key signed userOp", async () => {
    // moved to /test/bundler-integration/module/SessionValidationModules/ERC721SessionValidation.Module.specs.ts
  });
  ...
  it("should revert when userOp is for an invalid token", async () => {
    ...
    const transferUserOp = await makeERC721TransferUserOp(
      mockToken2.address,
    ...
    await expect(
      ...
      .withArgs(0, "AA23 reverted: ERC721SV Wrong Token");
  ...
  it("should revert if userOp calldata involves sending value", async () => {
    ...
    await expect(
      ...
      .to.be.revertedWith("FailedOp")
      .withArgs(0, "AA23 reverted: ERC721SV Non Zero Value");
  ...
  it("should revert if userOp is for wrong recipient", async () => {
    ...
    await expect(
      ...
      .to.be.revertedWith("FailedOp")
      .withArgs(0, "AA23 reverted: ERC721SV Wrong Recipient");
  ...
  it("should revert if userOp is for too large amount", async () => {
    ...
    await expect(
      ...
      .to.be.revertedWith("FailedOp")
      .withArgs(0, "AA23 reverted: ERC721SV Max Amount Exceeded");
    expect(await mockToken.balanceOf(charlie.address)).to.equal(
      charlieTokenBalanceBefore
    );
  });
  ...
  it("Should not allow to call methods except execute or execute_ncC", async () => {
    ...
    await expect(
      ...
      .to.be.revertedWith("FailedOp")
      .withArgs(0, "AA23 reverted: ERC721SV Invalid Selector");
    expect(await mockToken.balanceOf(charlie.address)).to.equal(
      charlieTokenBalanceBefore
    );
  });
  ...
  it("should be able to execute arbitrary method instead of `execute` on a vulnerable module", async () => {
    ...
    const { sessionKeyData, leafData } = await getERC721SessionKeyParams(
      ...
      0,
      0,
      vulnerableerc721SessionModule.address
    );
    ...
    const userSA_ = await ethers.getContractAt("IERC721", userSA.address);
